...
class Loss(_Loss):
    def __init__(self, conf):
        ...
        for idx, lossfun_name in enumerate(lossfun_names):
            for key in conf[lossfun_name].params.keys():
                if 'weight' in key and isinstance(type(conf[lossfun_name].params[key]), list):
                    conf[lossfun_name].params[key] = torch.tensor(conf[lossfun_name].params[key])
    ...
    def forward(self, pred, target):
        ...
        for key in self.lossfuns.keys():
            loss = self.lossfuns[key](pred, target)
